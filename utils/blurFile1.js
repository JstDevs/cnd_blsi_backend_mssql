...
async function convertPdfToImages(inputPdf, outputFolder) {
  ...
  await convert(inputPdf, options);
}
...
async function redactRegionsOnImage(buffer, outputfile,outputPath, regions) {
  console.log('🔍 Redacting regions on buffer:', buffer,"outputpath====>",outputPath,"<=======regions",regions);
...
// Main function
async function bluritout(INPUT_PDF, OUTPUT_PDF, TEMP_DIR, blurRegions,filepathrelativetoserver) {
  ...
  try {
    ...
    for (let i = 0; i < allPages.length; i++) {
      ...
      console.log(">>>>>>>>>>>>>>>>","imgpath",imgPath,"allpages[i]",allPages[i],"<<<<<<<<<<<")
      ...
      if (pageRegions.length) {
        ...
        const filetopush=filepathrelativetoserver+`/${filename}`
        processedImages.push(filetopush);
      } else {
        const filetopush=filepathrelativetoserver+`/${allPages[i]}`
        processedImages.push(allPages[i]); // Use original if no blur
      }
    }
